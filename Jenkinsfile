...
// which are part of the stone repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="stone-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /stone-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "stone-project-stone/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from stone-project-stone/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'stone-project-stone/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from stone-project-stone/develop"
            }
          }

          if (hasCache) {
            sh "cd /stone-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /stone-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /stone-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /stone-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /stone-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /stone-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /stone-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
