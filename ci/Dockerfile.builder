...
# stone_hash
RUN git clone https://github.com/stone-project/stone_hash
RUN cd stone_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} stone
RUN useradd -u ${USER_ID} -g stone -s /bin/bash -m -d /stone stone
...
RUN mkdir /stone-src && \
  ...
  chown $USER_ID:$GROUP_ID /stone-src && \
