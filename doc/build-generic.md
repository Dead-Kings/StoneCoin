GENERIC BUILD NOTES
====================
Some notes on how to build Stone Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build stone with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Stone Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
stone inherited the `depends` folder from Bitcoin, which contains all dependencies required to build stone. These
dependencies must be built before stone can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build Stone Core. Caching of build results will ensure that only
...
not done, build failures might occur when building stone.
...
ccache
------
`./configure` of Stone Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
