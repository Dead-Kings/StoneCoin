...
class BitcoinTestFramework(object):
    ...
    def main(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: stoneds were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a stoned test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple stoned test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'stoned exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "stoned should have exited with an error"
                else:
                    assert_msg = "stoned should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as stoned's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("stoneD", "stoned"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0", "-mocktime="+str(GENESISTIME)]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in stone.conf
...
class stoneTestFramework(BitcoinTestFramework):
    def set_stone_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "stoned"),
                          help="stoned binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "stoned"),
                          help="stoned binary to use for reference nodes (if any)")
